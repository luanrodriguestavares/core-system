...
class NullGateway implements BillingGatewayInterface
{
    public function createCustomer(Tenant $tenant, array $payload): array
    {
        return [
            'gateway_customer_id' => 'null_customer',
            'raw' => $payload,
        ];
    }
    ...
    {
        return [
            'gateway_subscription_id' => 'null_subscription',
            'status' => 'active',
            'raw' => [
                'plan' => $plan->id,
    ...
    {
        return [
            'event_id' => $request->header('X-Event-Id', 'null_event'),
